...
mkdir -p ${WORKDIR}/RF_diff/round2/
# Create necessary directories
mkdir -p ${WORKDIR}/RF_diff/round2/beta_weights
mkdir -p ${WORKDIR}/RF_diff/round2/normal_weights
# Part 1: Protein Backbone Generation


# ATP Binders with Beta Weights
RESULTS_DIR=${WORKDIR}/RF_diff/round2/beta_weights/
cd ${RESULTS_DIR}
...
RESULTS_DIR=${WORKDIR}/RF_diff/round2/normal_weights/
cd ${RESULTS_DIR}
"${RFdiff_path}" inference.output_prefix="${RESULTS_DIR}" inference.input_pdb="${INPUT_PDB}" \
    'contigmap.contigs=[A311-501/0  20/B26-29/47/B91-95/45/C100-105/45/C52-57/20]' \
    'ppi.hotspot_res=[A316,A325,A346,A348,A350,A363,A380,A382,A412,A417,A438,A467]' \
